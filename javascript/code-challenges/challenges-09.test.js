...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
Write a function that finds the maximum value in an array using the 'reduce' method.
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
...
const updateNumbers = (obj) => {
	const newArr = Object.entries(obj).map(entry => {
		const key = entry[0];
		const value = entry[1];
		// const [key, value] = entry; <-- can also use this 'destructuring' syntax
		return `${key}: ${value}`;
	});
	return newArr;
};
...
const hasChildrenValues = (arr, character) => {
	const charObj = arr.find(obj => obj.name == character);
	return 'children' in charObj;
};

// search through array to find a specific character
// if 'children' exists as a key, return true, if not, return false
